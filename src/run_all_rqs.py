...
def main():
    ...
    ap.add_argument("--with_meta", action="store_true")
    args = ap.parse_args()
    ...
    scripts = [
        ...
        ["python", "src/rq5_augmentation.py", "--data", args.data, "--epochs", str(args.epochs), "--batch_size", str(args.batch_size), "--out", args.out] + (["--run_meta"] if args.with_meta else []),
    ]

    for s in scripts:
        run(s)

    zp = zip_figures(Path(args.out), "Figures_Tables.zip")
    print("\nCreated:", zp)
